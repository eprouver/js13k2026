...
:root {
  ...
  --E: cubic-bezier(.4,.05,.2,1);
  --O: max(7.25vmin, 3rem);
  --Of: calc(var(--O) * .36);
  --dim: grayscale(1) brightness(.55);
  --line: .2rem;
  --Bl: .6rem;
...
.pmc {
  ...
  background: #222;
  animation: pmc-in .7s cubic-bezier(.2,1.4,.3,1) both;
}
...
.ct.sl {
  ...
  animation: tab-seal .8s var(--E) both;
}
...
.tps {
  ...
  width: 1.85rem;
  height: 1.85rem;
  transition: opacity .4s ease, transform .7s var(--E);
}
...
body.won #uni {
  animation: uni-gallop .74s ease infinite;
}
...
.ds::before {
  ...
  background: var(--slot);
  border-radius: 0;
  transition: width .8s var(--E) var(--d, 0s);
}
body.won .ds::before {
  left: -.35rem;
  width: 100vw;
...
.cf.pd { opacity: .85; }
.cf.lit { opacity: 1; }
.cf.lit.ig { animation: cube-ig .6s ease; }
...
.pmod.out .pmc {
  animation: pmc-out .5s ease both;
}
